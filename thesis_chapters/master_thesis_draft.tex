...
monocular learning-based pipeline (FastSAM3D) and a hybrid output that fuses
FastSAM3D with Viscando processing (Merge); an Xsens inertial motion-capture
...
The Merge hybrid does not preserve short-term motion structure (mean K=6
Pearson $\approx$0.01) despite plausible average amplitude, indicating that
current fusion strategies degrade temporal coherence rather than improving
it, and a dense stereo SGBM path is dropped from the main analysis because
low-texture clothing collapses disparity around the relevant joints.
SKT retains a clear position-space advantage because its 3D coordinates
inherit metric scale from calibrated stereo geometry, whereas FastSAM3D
requires external coordinate and scale alignment before position-space
comparison becomes meaningful; SKT also exposes explicit per-frame quality
signals (epipolar error, reprojection error, triangulation confidence) that
support quality gating in deployment.
These findings indicate that learning-based monocular pose models are
competitive on angle and motion agreement for this dataset, but stereo
geometry remains essential for absolute metric scale and for the
geometry-aware quality control that ergonomic monitoring deployments
require.
...
benchmarks SKT against a monocular learning-based pipeline (FastSAM3D), a
hybrid output that fuses FastSAM3D with Viscando processing (Merge), and a
dense stereo Semi-Global Block Matching (SGBM) alternative.
...
\begin{enumerate}[label=\textbf{RQ\arabic*:}, leftmargin=1.8cm]
  ...
  \item Under a unified evaluation protocol (shared timeline, common
        ...
        SKT pipeline compare with the learning-based FastSAM3D pipeline and
        the hybrid Merge output in (i) absolute angle accuracy, (ii)
        K-frame motion consistency at $K\in\{1,6,12,25\}$ frames, and (iii)
        reconstructed 3D position geometry?
...
Section~\ref{sec:methods} presents the four pose-estimation paths (SKT,
FastSAM3D, Merge, SGBM).
...
First, it benchmarks three independently produced vision pipelines
side-by-side on the same recording -- a calibrated sparse-stereo pipeline
(SKT), a monocular learning-based pipeline (FastSAM3D), and a hybrid
fusion output (Merge) -- so that the geometry-versus-learning tradeoff is
...
Appendix~A and are treated as fixed inputs for all SKT and SGBM
...
The thesis benchmarks four pose-estimation paths against the same Xsens
reference and the same evaluation protocol, and the four paths are chosen
so that the geometry-versus-learning axis and the sparse-versus-dense stereo
axis can both be examined.
SKT is the main contribution and represents calibrated sparse-stereo
geometry; FastSAM3D represents the monocular learning-based alternative;
Merge represents a hybrid that fuses FastSAM3D with Viscando processing;
and SGBM represents the dense-stereo alternative to SKT's sparse
triangulation.
Figure~\ref{fig:pipeline_overview} shows the full experimental flow, and
Table~\ref{tab:method_roles} summarises each path's input assumption, main
advantage, and main risk so that the per-method subsections below can stay
focused on the methodological detail.
...
All four methods are mapped onto the same shared timeline and judged with
the same metrics, but those metrics are then split across three independent
axes (angle, motion, position) rather than collapsed into a single ranking
...
\subsection{Merge (Viscando $\times$ FastSAM3D Hybrid)}

Merge is included as a third external pipeline that asks a single
methodological question: does fusing the FastSAM3D output with Viscando's
internal processing improve the agreement with the Xsens reference, or
does the fusion step degrade the per-frame motion signal even when the
average geometry looks plausible?
Merge is evaluated under exactly the same angle, motion, and position
protocols as SKT and FastSAM3D so that the comparison is fair; in the
current run, Merge does not improve the headline angle or motion-space
metrics (its K=6 motion correlation is near zero on the elbow), so it is
treated below as a diagnostic comparison rather than as a competing
recommended method.

\subsection{Dense Stereo SGBM (Alternative Stereo Path)}

SGBM is included to test the natural counter-hypothesis to SKT's sparse
triangulation: if SKT's main weakness is its dependence on a handful of
2D keypoints per frame, then a dense-stereo depth estimate evaluated at
those keypoints might in principle be more robust.
After stereo rectification, Semi-Global Block Matching produces a dense
disparity map across the image \cite{hirschmuller2007sgbm}, and each 2D
keypoint's local disparity is then converted to a depth value through the
stereo baseline and the rectified focal length, giving an alternative 3D
joint position that does not rely on left/right keypoint matching at all.

In the present recording the dense-stereo hypothesis fails for a specific
and well-understood reason rather than for a tunable parameter choice,
which is itself a useful negative result for the thesis.
Semi-Global Block Matching requires local image texture to find reliable
correspondences, but the dark workwear of the subject and the dark
furniture in the scene create texture-poor regions exactly where the
ergonomically relevant joints project; the disparity values therefore
either collapse to invalid or fluctuate strongly between frames, and the
resulting joint depths are not stable enough to support angle estimation
at the level required by ergonomic monitoring.
Figure~\ref{fig:ddm_diagnostics} reports the disparity fill-rate
diagnostic and a depth comparison scatter that document this failure
quantitatively; the dense-stereo path is therefore retained in the thesis
as a diagnostic alternative rather than as a candidate main method, and
this prevents the over-simplified conclusion that ``denser is better''
for industrial pose estimation.
...
The four evaluation tracks above are well-defined only if the per-system
...
is the strongest of the three evaluated vision methods, but the margin
over SKT is small enough that the result must be read with the Xsens
reference-system caveat in mind rather than as a universal verdict.
...
17.6\degr\ for SKT and 24.2\degr\ for the Merge hybrid; the corresponding
median MAEs (14.4\degr, 17.9\degr, 22.4\degr) preserve the same ranking,
and FastSAM3D also has the highest valid-pair ratio.
...
The Merge hybrid behaves qualitatively differently from both: its K=6
Pearson is essentially zero on the elbow, even though its angle MAE
(24.2\degr) and its average ROM amplitude in the segment analysis are not
catastrophic, which is the symptom of a fusion pipeline that preserves
average geometry while disrupting frame-to-frame temporal coherence.
...
consistently above SKT across the eight evaluated angles, while Merge sits
near zero throughout, and the Xsens-native check provides the visual
upper bound that anchors the scale of the plot.
...
Aggregated over the detected active segments (39 for SKT, 45 for the
external methods), FastSAM3D has both the lowest mean ROM error (14.9\degr)
and the lowest mean DTW distance (0.0355), compared with SKT (26.9\degr;
0.0435), Merge (30.0\degr; 0.0772), and the Xsens-native check
(4.8\degr; 0.0062).
Two patterns are worth highlighting before the cross-method synthesis.
First, Merge's high DTW distance reinforces its low K=6 Pearson: the
fusion process disrupts shape-of-motion agreement more strongly than it
disrupts mean amplitude, so a method that looks acceptable in segment
ROM alone (30.0\degr is comparable to SKT's 26.9\degr) is in fact further
from the reference in shape than the table's first column suggests.
Second, the Xsens-native DTW distance ($\sim$6$\times 10^{-3}$) sets the
lower bound that the normalised DTW metric can reach on this dataset, so
...
Reading the four tracks together produces a more nuanced verdict than any
of them gives on its own, and this is the central reason the thesis
chose a three-axis evaluation rather than a single-metric ranking.
Table~\ref{tab:cross_method_synthesis} consolidates the headline numbers
for the three vision methods (with the Xsens-native check shown alongside
as a reference-system floor).
The pattern that emerges is consistent across all four tracks but does
not produce a single winner.
FastSAM3D leads the angle-space (lower MAE) and motion-space (higher
correlation) tracks and also the segment-level shape and ROM tracks,
which together indicate that learned body and motion priors do suppress
the per-frame and per-segment jitter that hurts a frame-independent
sparse stereo reconstruction on this dataset.
SKT lags FastSAM3D on angle and motion but retains the position-space
track outright, because its 3D coordinates carry metric scale by
construction and because the per-frame geometric quality signals it
exposes (epipolar error, reprojection error, triangulation confidence)
are exactly the signals that downstream ergonomic quality gating
requires.

\begin{table}[!htbp]
  \centering
  \caption{Cross-method headline synthesis. Values consolidate the four evaluation tracks. Best vision-method value per column is highlighted; the Xsens-native row is a reference-system internal check, not a competing method.}
  ...
  \begin{tabular}{lcccccl}
    \toprule
    Method & Angle MAE & RULA bin & Motion $r$ & Segment ROM & Segment DTW & Metric scale \\
           & (\degr) $\downarrow$ & agree.\ $\uparrow$ & K=6 $\uparrow$ & err.\ (\degr) $\downarrow$ & dist.\ $\downarrow$ & \\
    \midrule
    SKT
    & 17.6 & 0.540 & 0.379 & 26.9 & 0.0435 & \good{Calibrated} \\
    \good{FastSAM3D}
    & \good{15.4} & \good{0.553} & \good{0.644} & \good{14.9} & \good{0.0355} & Needs alignment \\
    Merge
    & 24.2 & 0.440 & 0.006 & 30.0 & 0.0772 & Needs alignment \\
    Xsens native (ref.)
    & \textit{4.7} & \textit{0.886} & \textit{0.965} & \textit{4.8} & \textit{0.0062} & --- \\
    \bottomrule
  \end{tabular}%
  }
\end{table}

The two remaining methods support, rather than contradict, this verdict.
Merge fails on every track except segment count, demonstrating that
straightforward fusion of FastSAM3D with Viscando processing does not
recover the headline numbers on this recording and in fact actively
degrades short-term motion agreement.
SGBM (not in the table because its evaluation is qualitative on this
recording, see Section~\ref{sec:methods}) confirms that dense disparity
is not automatically better than sparse triangulation when the relevant
body regions lack reliable image texture.
The overall pattern is therefore: \emph{the learning-based monocular
pipeline wins the agreement tracks; the calibrated stereo pipeline wins
the deployment-geometry track; and neither replaces the other.}
This split, rather than a clean single-method recommendation, is the
substantive result of the cross-method synthesis and is what
...
learning-based monocular FastSAM3D pipeline delivers the better
...
26.9\degr) than the calibrated stereo SKT pipeline; the hybrid Merge
output fails to recover this advantage and in fact disrupts short-term
motion coherence, and the dense-stereo SGBM alternative is dropped from
the main analysis because dark workwear collapses disparity around the
relevant joints.
...
that all SKT and SGBM experiments in the thesis use as fixed input; the
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{p{4.0cm}Xp{1.9cm}}
    ...
    Merge does not preserve short-term motion structure.
    & Table~\ref{tab:motion_summary} and Table~\ref{tab:cross_method_synthesis}: mean K=6 Pearson $\approx$0.006 despite plausible mean ROM amplitude.
    & Supported \\
    The cross-method ranking is robust to smoothing and segmentation parameters.
    & Appendix~\ref{app:sensitivity}: sensitivity tables across smoothing windows and activity thresholds preserve the FastSAM3D $>$ SKT $>$ Merge ranking.
    & Supported \\
