...
<section id="smpl"><h2>6. EasyMocap / SMPL 准入结果</h2><p>官方 SMPL v1.1 neutral 资产通过 CUDA forward。Fanbo7 A257 连续 40 帧使用固定标定、单 session shape、双视图重投影、质量加权 SKT anchor、CMU GMM pose prior 和时间连续性完成拟合；Xsens-derived reference 未参与初始化或选参。</p><table><tr><th>指标</th><th>SMPL 结果</th></tr><tr><td>核心关节有效率</td><td>1.0000</td></tr><tr><td>重投影 p50 / p95</td><td>10.751 / 35.336 px</td></tr><tr><td>骨长 CV</td><td>0.0121</td></tr><tr><td>高质量点修正 median / p95</td><td>6.154 / 28.695 cm</td></tr><tr><td>先验耗时 / 估算端到端</td><td>404.22 ms/帧 / 2.30 fps</td></tr></table><p class="bad">结论：人体模型完整且骨长稳定，但它把可靠图像观测拉开数厘米，形成“人体合理但动作不一致”的解。几何与实时门槛同时失败，未继续 Fanbo4、200 帧或全量测试；拟合适配器已回滚，许可资产验证环境保留。</p></section>
<section id="conclusion"><h2>7. 结论与下一步</h2><figure><img src="{pareto}"><figcaption>图 3：可比较候选的精度—速度 Pareto 图。Pose2Sim 横坐标仅按 IK 阶段耗时换算，是未计入检测与三角化的乐观上界；即便如此仍被 SKT 基线支配。MeTRAbs、轻量运动学先验和 SMPL 均因几何门槛失败，没有可报告的有效角度精度点。</figcaption></figure><ol><li>当前正式基线继续使用确定性 PyTorch FP32 YOLOv8m + SKT。</li><li>不继续重复已拒绝的 SMPL/运动学权重网格；负结果与可视化证据保留。</li><li>下一阶段优先改善遮挡和左右语义错配下的双目 2D 观测质量，再考虑较弱的关节限位。</li><li>FoundationStereo 可用于遮挡区深度诊断，但不能替代关键点语义。</li><li>所有调参继续只依赖几何和时间稳定性；Xsens 仅用于最终 agreement 报告，避免误差转移。</li></ol></section>"""
    else:
...
<section id="smpl"><h2>6. EasyMocap / SMPL gate</h2><p>The official SMPL v1.1 neutral asset passed its CUDA forward validation. Forty continuous Fanbo7 A257 frames were fitted with the frozen calibration, one session shape, two-view reprojection, a quality-weighted SKT anchor, the CMU GMM pose prior, and temporal continuity. Xsens-derived data were not used for initialization or selection.</p><table><tr><th>Metric</th><th>SMPL result</th></tr><tr><td>Finite core-joint ratio</td><td>1.0000</td></tr><tr><td>Reprojection p50 / p95</td><td>10.751 / 35.336 px</td></tr><tr><td>Bone-length CV</td><td>0.0121</td></tr><tr><td>High-quality correction median / p95</td><td>6.154 / 28.695 cm</td></tr><tr><td>Prior time / estimated end-to-end</td><td>404.22 ms/frame / 2.30 fps</td></tr></table><p class="bad">Rejected on geometry and real-time criteria. The prior produced a complete, stable body but displaced reliable image observations by several centimeters. Fanbo4 and longer gates were not run; the fitting adapter was reverted while the licensed-asset validation environment was retained.</p></section>
<section id="conclusion"><h2>7. Conclusions and next steps</h2><figure><img src="{pareto}"><figcaption>Figure 3. Accuracy-throughput Pareto view for comparable candidates. Pose2Sim throughput is an optimistic upper bound derived from IK-stage time alone; it still remains dominated by the SKT baseline. MeTRAbs, the lightweight kinematic prior, and SMPL have no valid accuracy points because they failed geometry gates.</figcaption></figure><ol><li>Retain deterministic PyTorch FP32 YOLOv8m + SKT as the formal baseline.</li><li>Do not repeat the rejected SMPL or kinematic weight grids; retain their negative evidence.</li><li>Prioritize stereo 2D observation quality under occlusion and left/right semantic mismatch before adding weaker joint limits.</li><li>Use FoundationStereo only as an occlusion/depth diagnostic, not as a joint-semantic replacement.</li><li>Select parameters from geometry and temporal stability; use Xsens only for final agreement reporting.</li></ol></section>"""
