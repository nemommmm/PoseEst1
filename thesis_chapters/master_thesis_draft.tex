...
offset by differencing and provide a short-term agreement measure that is
less sensitive to static offsets.
On the primary 2025 single-subject manufacturing recording of 2801 synchronized stereo frame
...
A supplementary 2026 detector-backbone analysis on 1219 common valid frames
finds a lower pooled median right-elbow disagreement for YOLOv8m than for
YOLO11L (17.58\degr\ vs.\ 18.49\degr) and does not support a fixed
near/far model-switching rule.
This angle and motion pattern is consistent with FastSAM3D's learned
body-shape and pose priors reducing the effect of per-frame keypoint jitter.
Because no prior ablation is available, that mechanism is treated as a
plausible explanation rather than as a measured cause.
...
throughout the thesis because it reduces the angle-definition mismatch
between systems.  Its agreement with the native representation
(MAE $\approx$ 2\degr\ in motion-space on the present recording) is reported
as an internal representation self-consistency check; it does not establish
a physical accuracy bound for either Xsens representation.
...
concern: the thesis quantifies agreement between two Xsens angle
representations on the same metrics that are applied to the vision
pipelines, so that definition-dependent disagreement remains visible rather
than being mistaken for physical accuracy.
...
The primary cross-method results in this thesis are computed on one
single-subject ergonomics recording collected in a controlled
manufacturing-like environment.  A separate 2026 Assar subset is used only
for the supplementary detector-backbone and distance-robustness study; it is
not mixed into the main SKT--FastSAM3D ranking.
...
\subsection{2026 Assar Supplementary Detector Dataset}
\label{sec:assar_supplement}

The supplementary detector study uses four synchronized stereo recordings
from the 2026 Assar collection: Fanbo7 A257, Fanbo9 A257, Fanbo9 A255, and
Fanbo4 A257.  These recordings contain the same operator but span two
calibrated camera pairs and median stereo-estimated optical depths of
approximately 2.09, 2.68, 3.07, and 4.03\,m.  The analysis is restricted to
the right elbow, which is the only ergonomic angle configured consistently
across all four recordings, and compares YOLOv8m with YOLO11L inside the same
SKT V2 geometry and filtering pipeline.

Each recording has its own Xsens MVNX file, used only as an Xsens-derived
external comparison reference.  The supplementary analysis does not use
Xsens to tune detector parameters or to choose a separate alignment for each
model; instead, it fixes one offset per recording before the paired detector
comparison.  Because recording, action, viewpoint, and distance change
together, this subset can reveal an association with estimated depth but
cannot isolate physical distance as a causal factor.
...
First, the elbow numbers provide a \emph{representation self-consistency
check}: they quantify how much two angle definitions derived from the same
Xsens recording disagree.  They help identify definition sensitivity but do
not measure the physical accuracy or information limit of Xsens.
...
The YOLOv8m--YOLO11L comparison is a supplementary detector-backbone ablation
inside SKT, not a third pose-estimation pipeline; all downstream geometry and
angle definitions are held fixed for that study.
...
\begin{table}[!htbp]
  ...
  \begin{tabularx}{\linewidth}{lXXX}
    ...
    & Stable angle and motion behaviour in the evaluated output
...
upper-body joints.  Distributional reporting additionally includes the
interquartile range (IQR), P90/P95, valid-pair count, scatter plots, and box
plots so that a small number of severe outliers cannot be hidden inside one
average.

\subsection{Distance-stratified Detector Analysis}
\label{sec:distance_method}

The supplementary detector analysis evaluates YOLOv8m and YOLO11L on exactly
the same valid frames and uses one fixed Xsens time offset per recording.
For each frame, optical depth is estimated from the median 3D position of the
YOLOv8m torso joints in the left-camera coordinate system; this same depth is
then assigned to both model rows so that an erroneous YOLO11L reconstruction
cannot move its own observation along the horizontal axis.  The distance
axis is divided into 0.5\,m bins, and a bin is admitted to the detector
comparison only when it contains at least 20 common valid frames.  Median,
IQR, P95, sample count, and paired per-frame absolute-angle differences are
reported against the Xsens-derived reference.  The resulting optical depth
is an estimated geometric quantity rather than an independent tape-measure
distance, which limits the analysis to association rather than causation.
...
Shoulder, hip, and knee angles dominate FastSAM3D's lead.  This pattern is
consistent with body priors stabilising chain endpoints, but no ablation
isolates that mechanism; the elbow is the
...
\begin{figure}[!htbp]
  ...
  \caption{Angle-space summary across evaluated methods. FastSAM3D has the lowest mean and median MAE among the vision methods, while the Xsens-native row is an internal comparison between two Xsens angle representations.}
...
The Xsens-native DTW distance ($\sim$6$\times 10^{-3}$) is included as an
internal representation self-consistency check.  FastSAM3D's
$\sim$3.5$\times 10^{-2}$ is larger, but this ratio is not interpreted as a
physical accuracy bound because both Xsens angle streams originate from the
...
\begin{figure}[!htbp]
  ...
  \caption{Segment-level ROM error and DTW distance. FastSAM3D gives the strongest segment-level agreement among the vision methods, while the Xsens-native row is an internal representation self-consistency check.}
...
\subsection{Supplementary Distance-stratified Detector Results}
\label{sec:distance_results}

The detector-backbone study retains 1219 frames on which YOLOv8m, YOLO11L,
the shared optical-depth estimate, and the Xsens-derived right-elbow
reference are all valid.  On this common set, the pooled median absolute
disagreement is 17.58\degr\ for YOLOv8m and 18.49\degr\ for YOLO11L.  The
pooled number is not treated as sufficient by itself because the recordings
contribute different frame counts; Table~\ref{tab:distance_detector_summary}
therefore reports each recording separately with its median, P95 tail
statistic, and common-frame count, while the figures show the IQR.

\begin{table}[!htbp]
  \centering
  \caption{Right-elbow detector-backbone comparison on common valid frames. Absolute disagreement is measured against the Xsens-derived external reference; lower is better.}
  \label{tab:distance_detector_summary}
  \small
  \begin{tabular}{lrrrr}
    \toprule
    Recording & Depth (m) & Common $n$ & v8m med./P95 & 11L med./P95 \\
    \midrule
    Fanbo7 A257 & 2.09 & 185 & 6.66 / 28.30 & 7.19 / 24.27 \\
    Fanbo9 A257 & 2.68 & 676 & 19.22 / 32.46 & 19.42 / 31.79 \\
    Fanbo9 A255 & 3.07 & 78 & 18.53 / 44.20 & 33.02 / 47.23 \\
    Fanbo4 A257 & 4.03 & 280 & 9.68 / 31.00 & 16.64 / 40.10 \\
    \bottomrule
  \end{tabular}
\end{table}

The distributional view changes the interpretation of the short-range
result.  On Fanbo7, YOLO11L has the lower mean absolute disagreement
(9.18\degr\ vs.\ 10.43\degr), but YOLOv8m has the lower median
(6.66\degr\ vs.\ 7.19\degr).  Thus the apparent YOLO11L advantage in the
mean is driven by its smaller upper tail rather than by a generally lower
typical-frame error.  Across the five 0.5\,m bins that meet the 20-frame
minimum, YOLO11L has the lower paired median in only one bin; the paired
YOLO11L-minus-YOLOv8m median changes from $-1.04$\degr\ at 2.0--2.5\,m to
$+8.61$, $+5.68$, and $+5.93$\degr\ in the three bins spanning
3.0--4.5\,m.

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=\linewidth]{thesis_detector_distance_scatter.png}
  \caption{Per-frame right-elbow absolute disagreement versus the shared YOLOv8m-derived optical-depth estimate. Points identify recordings; thick lines and bands show 0.5\,m-bin medians and IQRs. The broad within-depth spread indicates that factors beyond estimated depth remain important, but this dataset does not isolate them.}
  \label{fig:detector_distance_scatter}
\end{figure}

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=\linewidth]{thesis_detector_distance_boxplot.png}
  \caption{Box plots of right-elbow disagreement by estimated optical-depth bin. The boxes expose the middle 50\% of frames and the outlier tail that is hidden by a single average.}
  \label{fig:detector_distance_boxplot}
\end{figure}

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.95\linewidth]{thesis_detector_paired_difference.png}
  \caption{Same-frame paired detector comparison. The vertical axis is YOLO11L absolute disagreement minus YOLOv8m absolute disagreement, so only negative values favour YOLO11L; bars show paired medians and whiskers show P25--P75.}
  \label{fig:detector_paired_difference}
\end{figure}
...
for SKT and FastSAM3D (with the Xsens-native check shown alongside as an
internal representation comparison).
FastSAM3D leads on the angle-space (lower MAE) and motion-space (higher
correlation) tracks.  This result is consistent with learned body and motion
priors reducing per-frame jitter, although the present comparison does not
isolate that mechanism from other model and preprocessing differences.
...
\begin{table}[!htbp]
  ...
  \begin{tabularx}{\linewidth}{lcccc>{\raggedright\arraybackslash}X}
    \toprule
    Method & Angle MAE & RULA agree. & Motion $r$ K=6 & Seg. ROM & Metric scale \\
    \midrule
    SKT
...
\subsection{A Plausible Explanation for FastSAM3D's Agreement Results}

One plausible reason FastSAM3D leads SKT on the angle and motion tracks is
that its learned body-structure and pose priors reduce the effect of the
per-frame failure mode that hurts a frame-independent triangulation
pipeline.  The experiment compares complete pipelines rather than ablating
the priors, so the mechanism cannot be identified causally.
FastSAM3D's 3D output is conditioned on body-shape plausibility constraints
learned from pose data, which could turn a momentarily unreliable image cue
into a smoother anatomical estimate rather than a sharp angle spike.  A
pattern consistent with that explanation is visible in the K=6 elbow scatter plots
...
\subsection{Detector Selection Across Estimated Depth}
\label{sec:distance_discussion}

Increasing camera-to-subject distance is expected to reduce the number of
pixels occupied by each limb and to make stereo depth more sensitive to a
one-pixel keypoint displacement.  The supplementary analysis supports a
distance association for detector robustness, but it does not show a smooth
causal disagreement--distance curve.  For example, the YOLOv8m median is
9.68\degr\ in the approximately 4.03\,m Fanbo4 recording but
19.22\degr\ in the approximately 2.68\,m Fanbo9 A257 recording.  This
non-monotonicity is consistent with viewpoint, action, self-occlusion, and
left--right semantic consistency contributing alongside distance.

The same distributional evidence also corrects the initial product-level
interpretation that YOLO11L should be selected automatically at short range.
Fanbo7 is the sequence on which the mean appears to favour YOLO11L, yet its
median favours YOLOv8m; across all admitted depth bins, only one of five has a
lower YOLO11L paired median, while all three bins between 3.0 and 4.5\,m
favour YOLOv8m.  The present evidence therefore supports retaining YOLOv8m
as the default detector, but it does not support a fixed distance threshold
for switching to YOLO11L.

A dynamic detector policy remains a reasonable engineering hypothesis, but a
future policy should be evaluated prospectively and should probably combine
estimated distance with directly observed quality signals such as person
bounding-box height, right-arm confidence, epipolar error, reprojection
error, and cross-model disagreement.  A controlled recording at 0.5\,m
increments with the same action and viewpoint would be required to define a
threshold without confounding it with session identity.  Until such a study
is available, the distance-based switch belongs in the design discussion,
not in the list of validated findings.

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.95\linewidth]{thesis_detector_mean_vs_median.png}
  \caption{Mean versus median right-elbow absolute disagreement for each recording and detector. Large gaps identify sequences in which a small number of severe frames changes the mean and can even alter the apparent model ranking.}
  \label{fig:detector_mean_vs_median}
\end{figure}
...
\begin{figure}[!htbp]
  ...
  \end{subfigure}
  \caption{Representative SKT high-delta diagnostic frames projected onto the original video view. In these examples, large motion deltas coincide with local keypoint instability around the elbow chain rather than with visibly large whole-body motion.}
...
\emph{First}, the primary SKT--FastSAM3D ranking comes from one subject and
one controlled 2025 recording.  The supplementary detector study adds a
second subject and four 2026 stereo recordings, but their actions,
viewpoints, and distances are not controlled independently.  Neither study
therefore supports a general statement about every operator, clothing style,
lighting condition, viewpoint, or camera distance.
...
their Xsens representation self-consistency is less well characterised.
...
reference and not as physical ground truth; the absolute angle disagreements are
therefore interpreted alongside the Xsens representation self-consistency
check established in Section~\ref{sec:data_setup} rather than as direct error
against a true biomechanical zero.

\subsection{Implications for Real-time Deployment}

The calibrated SKT pipeline has now been exercised on an NVIDIA RTX A6000
using deterministic PyTorch FP32.  In offline dual-file tests, the measured
preliminary single-run offline-file processing throughput is 28.88\,fps on
Fanbo7 and 29.23\,fps on Fanbo4; four complete-file runs span
28.17--29.92\,fps.  The timed path includes file decoding, YOLO inference,
tracking, and per-frame stereo geometry.  These measurements exceed the
12.5\,fps camera rate, but they do not yet represent a complete live-camera
system because acquisition, display, and downstream output are not all in
the measured loop.

An Ultralytics TensorRT FP16 export increased preliminary Fanbo7 file-processing throughput only
to 30.21\,fps while changing the RightElbow MAE from 10.43\degr\ to
12.81\degr\ and reducing the valid ratio from 0.427 to 0.289.  It therefore
failed the output-equivalence check (frame coverage, angle agreement,
validity, and RULA-like agreement relative to PyTorch) and is not the accepted deployment
baseline.  The immediate next step is not further speed tuning on that
failed engine, but a complete capture-to-output measurement using
NVDEC/GStreamer or DeepStream, followed by target-hardware tests.  NVIDIA
TAO BodyPoseNet and BodyPose3DNet are relevant detector candidates, but they
must first pass the same stereo semantics, angle, and validity gates; no
A6000 result can be used to infer a reliable throughput for an unspecified
RTX 30-series GPU or Jetson device.
...
The supplementary 2026 detector study does not establish a distance-based
switching rule: YOLOv8m has the lower pooled median right-elbow disagreement
(17.58\degr\ vs.\ 18.49\degr), and YOLO11L has the lower paired median in
only one of five sufficiently populated optical-depth bins.
...
\begin{enumerate}[leftmargin=1.2cm]
  \item \textbf{Controlled validation on additional recordings and subjects.}
        Repeat the same three-axis protocol with additional operators,
        clothing, lighting, and unscripted tasks.  Add a controlled
        distance series from 2.0 to 4.5\,m in 0.5\,m steps while holding
        action and viewpoint fixed, so that a detector-switching threshold
        can be tested without confounding distance with recording identity.
  \item \textbf{Real-time GPU deployment of SKT with optional learned
        stabilisers.} Retain deterministic PyTorch FP32 as the accepted
        A6000 baseline, complete the live capture/decode/display path with
        NVDEC/GStreamer or DeepStream, and measure capture-to-output
        latency.  Any future TensorRT or NVIDIA TAO candidate must pass the
        same keypoint, angle, validity, and RULA-equivalence gates before
        its speed result is considered deployable.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}p{3.8cm}>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}p{2.0cm}}
    \toprule
    ...
    & Present recording \\
    SKT high-delta points coincide with local keypoint-chain instability.
    ...
    & Qualitative \\
    Estimated optical depth is associated with the detector disagreement distribution.
    & Table~\ref{tab:distance_detector_summary}, Figure~\ref{fig:detector_distance_scatter}, and Figure~\ref{fig:detector_distance_boxplot}; the distributions differ across depth bands, but session, action, viewpoint, and occlusion are confounded.
    & Association only \\
    A fixed distance threshold should switch from YOLO11L at short range to YOLOv8m at long range.
    & Figure~\ref{fig:detector_paired_difference}: YOLO11L has the lower paired median in only one of five admitted bins, and no independent controlled-distance validation has been performed.
    & Not validated \\
