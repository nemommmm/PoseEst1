...
# PyTorch 2.x requires split sizes to be a Python list rather than a tensor.
sed -i \
  's/torch.split(poses3d_flat, n_box_per_image)/torch.split(poses3d_flat, n_box_per_image.tolist())/' \
  "${EXTERNAL_ROOT}/metrabs/metrabs_pytorch/multiperson/multiperson_model.py"
sed -i \
  's/torch.split(poses2d_flat, n_box_per_image)/torch.split(poses2d_flat, n_box_per_image.tolist())/' \
  "${EXTERNAL_ROOT}/metrabs/metrabs_pytorch/multiperson/multiperson_model.py"
