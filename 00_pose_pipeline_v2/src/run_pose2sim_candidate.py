...
def main() -> None:
    ...
    with np.load(args.skt, allow_pickle=False) as payload:
        ...
        all_keypoints = np.asarray(payload["keypoints"], dtype=np.float64)
        filled_all, interpolation_mask_all = fill_trajectories(all_keypoints)
        timestamps = np.asarray(payload["timestamps"][:stop], dtype=np.float64)
        raw_keypoints = all_keypoints[:stop]
    ...
    filled = filled_all[:stop]
    interpolation_mask = interpolation_mask_all[:stop]
