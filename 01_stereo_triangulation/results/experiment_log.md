...
## 2026-05-25 — SKT-Only Deep Dive Without FastSAM3D Fusion

Snapshot before experiments: `fe38a15 chore: snapshot before SKT-only enhancement deep dive`.

Motivation: FastSAM3D prior fusion was removed from the main ranking because it
is not a clean SKT improvement. It remains a diagnostic experiment only.

### Methodology Fix

- Updated `22_elbow_quality_repair.py` to use corrected stereo metadata timeline by default.
- Updated `24_temporal_smoothing_ablation.py` to use corrected stereo metadata timeline by default.
- Reason: the original SKT NPZ timestamps contain `241` non-positive frame-to-frame differences.

### Quality-Aware Repair, Corrected Timeline

- Output: `01_stereo_triangulation/results/skt_model_fusion/elbow_quality_repair_gap12_corrected_timeline/`
- Gap: `12` frames.
- Timeline: corrected.

Reference-free high-delta result:
...
Interpretation: this is currently the cleanest SKT-only improvement.

### OneEuro / Bone Prior / Kalman

- Output: `01_stereo_triangulation/results/skt_model_fusion/temporal_smoothing_ablation_full_raw/`
- Input: full-sequence `raw_pose.npz`.
- Timeline: corrected.

Key finding: Kalman looks strong on internal high-delta counts but fails the
downstream shape/agreement check. It over-smooths or predicts through missing
segments.
...
### Updated SKT-Only Ranking

1. Quality-aware repair with corrected timeline.
2. Existing OneEuro / bone+OneEuro as baseline stabilization.
3. YOLO11m full-sequence test only if time allows.
4. RTMPoseS direct replacement not recommended without retuned gates.
