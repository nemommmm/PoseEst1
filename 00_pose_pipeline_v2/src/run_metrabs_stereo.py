...
def load_model(model_dir: Path):
    ...
    with torch.device("cuda"):
        estimator = Pose3dEstimator(crop_model, skeleton_infos, transform)
    # The upstream wrapper always constructs an Ultralytics detector. Calling
    # parent ``eval()`` would invoke Ultralytics' public training API instead of
    # nn.Module.train(False). We supply trusted SKT boxes, so remove the unused
    # detector before moving/evaluating the wrapper.
    estimator.detector = torch.nn.Identity()
    ...
    try:
        for frame_idx in range(limit):
            ...
            with torch.inference_mode(), torch.device("cuda"):
                prediction = model.estimate_poses_batched(
                    frames,
                    boxes,
                    intrinsic_matrix=intrinsics,
                    distortion_coeffs=np.zeros((2, 5), dtype=np.float32),
